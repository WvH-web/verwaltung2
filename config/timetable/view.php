...
if ($planId) {
    ...
    $sql = "SELECT te.id, te.weekday, te.time_start, te.time_end,
                   te.period_start, te.is_double_first, te.is_double_second,
                   t.id AS teacher_id,
    ...
    <div class="table-responsive">
      <table class="table mb-0" style="min-width:580px;table-layout:fixed">
        ...
        <tbody>
          ...
          <?php else: ?>
          <?php
            $slotEntries = array_filter($entries, fn($e) => $e['time_start'] === $slot['start']);
          ?>
          <tr style="border-top:1px solid #e9ecef">
            ...
            <?php for ($d = 1; $d <= 5; $d++):
              ...
              $dayEntries = array_values(array_filter($slotEntries, fn($e) => $e['weekday'] == $d));
            ?>
            <td class="p-1 border-0" style="vertical-align:top;<?= $isToday ? 'background:#f0f7ff' : '' ?>">
              <?php foreach ($dayEntries as $e):
                $col = $subjectColors[$e['subject_name']] ?? '#2d6fa4';
                $isDS = $e['is_double_first'] || $e['is_double_second'];
              ?>
              <div class="rounded mb-1" style="background:<?= $col ?>14;border-left:3px solid <?= $col ?>;padding:3px 5px;font-size:.7rem;line-height:1.35">
                ...
                <?php if ($mode === 'klasse'): ?>
                  <div class="text-truncate" style="color:#555;font-size:.65rem"><?= e($e['teacher_name']) ?></div>
                <?php elseif ($e['classes']): ?>
                  <div class="text-truncate" style="color:#555;font-size:.65rem"><?= e($e['classes']) ?></div>
                <?php endif; ?>
              </div>
              <?php endforeach; ?>
            </td>
