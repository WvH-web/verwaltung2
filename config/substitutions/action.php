...
if ($action === 'release') {
    ...
    try {
        ...
        // Existiert schon eine offene Vertretung?
        $existing = $db->prepare(
        ...
        if ($existing->fetchColumn()) {
            $db->rollBack();
            setFlash('warning', 'Für diese Stunde gibt es bereits eine offene Vertretung.');
            header('Location: '.$back); exit;
        }
        ...
        $db->commit();
        logAudit(Auth::userId(), 'substitution.released', 'lesson_instances', $lessonId);
        setFlash('success', 'Stunde vom '.date('d.m.Y', strtotime($date)).' freigegeben.');
    } catch (\Exception $e) {
        $db->rollBack();
...
if ($action === 'claim') {
    ...
    if ((int)$sub['original_teacher_id'] === $myTid && !Auth::isVerwaltung()) {
    ...
    logAudit(Auth::userId(), 'substitution.claimed', 'substitutions', $subId);
    ...
    try {
        ...
        foreach ($admins as $adm) {
            notify($db, $subId, $adm['id'], 'pending_confirm',
                Auth::userName().' hat sich als Vertreter eingetragen (warte auf Bestätigung).');
        }

        logAudit(Auth::userId(), 'substitution.self_assigned', 'substitutions', $subId);
...
if ($action === 'confirm') {
    ...
    if (!$sub || ((int)$sub['original_teacher_id'] !== $myTid && !Auth::isVerwaltung())) {
    ...
    logAudit(Auth::userId(), 'substitution.confirmed', 'substitutions', $subId);
...
if ($action === 'reject') {
    ...
    logAudit(Auth::userId(), 'substitution.rejected', 'substitutions', $subId);
...
if ($action === 'admin_confirm') {
    ...
    try {
        ...
        $db->commit();
        logAudit(Auth::userId(), 'substitution.admin_confirmed', 'substitutions', $subId);
        setFlash('success', 'Vertretung bestätigt.');
    } catch (\Exception $e) {
        $db->rollBack();
...
if ($action === 'admin_direct') {
    ...
    try {
        ...
        // Existierende offene Vertretung löschen oder neu erstellen
        $ex = $db->prepare(
            "SELECT id FROM substitutions WHERE lesson_id=? AND status NOT IN ('confirmed','locked','cancelled') LIMIT 1"
        );
        $ex->execute([$lessonId]);
        ...
        if ($existSubId) {
            $db->prepare(
                "UPDATE substitutions SET substitute_teacher_id=?, status='confirmed',
                 ...
                 WHERE id=?"
            )->execute([$newTeachId, Auth::userId(), $notes ?: null, $existSubId]);
            $subId = $existSubId;
        } else {
            $db->prepare(
                "INSERT INTO substitutions
        ...
        logAudit(Auth::userId(), 'substitution.admin_direct', 'substitutions', $subId);
