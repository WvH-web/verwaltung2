...
if ($planId) {
    ...
    $sql = "SELECT
                ...
                te.is_double_second,
                te.double_group_id,
                (SELECT te2.id FROM timetable_entries te2 WHERE te2.double_group_id=te.double_group_id AND te2.id<>te.id LIMIT 1) AS double_partner_entry_id,
                (SELECT te2.time_start FROM timetable_entries te2 WHERE te2.double_group_id=te.double_group_id AND te2.id<>te.id LIMIT 1) AS double_partner_time_start,
...
// Build partner lookup: entry_id → partner lesson row (keyed by weekday+double_group_id)
// So DS1 knows its DS2 data and vice versa
$dsPartnerByEntry = []; // [entry_id => partner_row]
foreach ($lessons as $l) {
    if (!$l['double_group_id']) continue;
    $dsPartnerByEntry[(int)$l['entry_id']] = null; // init
}
foreach ($lessons as $l) {
    if (!$l['double_group_id']) continue;
    foreach ($lessons as $m) {
        if ($m['double_group_id'] === $l['double_group_id']
            && $m['entry_id'] !== $l['entry_id']
            && $m['weekday'] === $l['weekday']) {
            $dsPartnerByEntry[(int)$l['entry_id']] = $m;
            break;
        }
    }
}
$dsRenderedAsPartner = []; // entry_ids that were rendered inside their partner's slot
...
// Build rowspan map: [weekday][slotStart] = true  → this cell needs rowspan=2
// Build skip map:   [weekday][slotStart] = true  → skip <td> (covered by rowspan above)
$slotKeys = array_keys($slots);
$dsRowspan = []; // [wd][slotStart] → true
$dsSkip    = []; // [wd][slotStart] → true
foreach ($lessons as $l) {
    if (!$l['is_double_first'] || !$l['double_group_id']) continue;
    $wd = (int)$l['weekday'];
    $partner = $dsPartnerByEntry[(int)$l['entry_id']] ?? null;
    if (!$partner) continue;
    // Find index of DS1 slot and DS2 slot
    $ds1Slot = $l['time_start'];
    $ds2Slot = $partner['time_start'];
    $idx = array_search($ds1Slot, $slotKeys);
    if ($idx !== false && isset($slotKeys[$idx+1]) && $slotKeys[$idx+1] === $ds2Slot) {
        $dsRowspan[$wd][$ds1Slot] = true;
        $dsSkip[$wd][$ds2Slot]   = true;
    }
}
...
.lesson-slot.ds-first  { border-radius:5px 5px 0 0; margin-bottom:0; border-bottom:2px dashed rgba(0,0,0,.13) !important; }
.lesson-slot.ds-second { border-radius:0 0 5px 5px; margin-top:0; }
      ...
      <table class="table table-bordered mb-0" style="min-width:560px;table-layout:fixed">
        ...
        <tbody>
          ...
          <tr>
            ...
            ?>
            <?php
              $needRowspan = !empty($dsRowspan[$d][$slotStart]);
              $needSkip    = !empty($dsSkip[$d][$slotStart]);
              if ($needSkip) { continue; } // TD already covered by rowspan above
            ?>
            <td class="p-1 align-top <?= $isFree?'freecol':($isToday?'today-col':'') ?>"
                <?= $needRowspan ? 'rowspan="2"' : '' ?>
                style="<?= $needRowspan ? 'vertical-align:top' : '' ?>">
              <?php if (empty($cells)): ?>
                <div style="min-height:36px"></div>
              <?php endif; ?>
              <?php foreach ($cells as $cell):
                // Skip DS2 if rendered inline below DS1
                if ($cell['is_double_second'] && isset($dsRenderedAsPartner[(int)$cell['entry_id']])): ?>
                  <?php continue; endif;
                ...
                $canRelease    = !$isFree
                                 && (!$ss || ($isConfirmed && Auth::isVerwaltung()))
                                 && (Auth::isVerwaltung()
                                     || ($isMine && Auth::isLehrer() && !$ss));
                $canClaim      = $ss === 'open' && !$isMine
                                 && (Auth::isLehrer() || Auth::isVerwaltung());
                $canSelfAssign = !$isFree && !$isMine
                                 && (!$ss || ($isConfirmed && Auth::isVerwaltung()))
                                 && (Auth::isLehrer() || Auth::isVerwaltung());
                $canConfirmMe  = $ss === 'pending_confirm' && $isMine;
                $canAdminOK    = Auth::isVerwaltung() && $ss === 'pending_confirm';
                $canReassign   = Auth::isVerwaltung() && $isConfirmed;
              ?>
              <?php $useRowspan = $cell['is_double_first'] && !empty($dsRowspan[$d][$slotStart]); ?>
              <div class="lesson-slot <?=$cls?> <?=$isMine?'mine':''?> <?=(!$useRowspan && $cell['is_double_first'])?'ds-first':'' ?><?=(!$useRowspan && $cell['is_double_second'])?'ds-second':'' ?>">
                ...
                <div class="mt-1 d-flex flex-wrap gap-1">
                  ...
                  <button type="button"
                          ...
                          onclick="openRelease(<?=(int)$cell['entry_id']?>,'<?=$ds?>',
                            ...
                            <?=(int)($cell['is_double_first']??0)?>,
                            <?=(int)($cell['is_double_second']??0)?>,
                            <?=(int)($cell['double_partner_entry_id']??0)?>,
                            '<?=substr($cell['double_partner_time_start']??'',0,5)?>')"
                  ...
                  <button type="button"
                          class="btn btn-xs btn-outline-warning"
                          onclick="openAdminAssign(<?=(int)$cell['sub_id']?>,'<?=addslashes($cell['subject_name'])?>','<?=$ds?>')"
                          title="Vertretung neu zuweisen">
                    <i class="bi bi-arrow-repeat"></i> Neu vergeben
                  </button>
                  <?php endif; ?>
              ...
              <?php
              // If DS1: render DS2 partner immediately below in the same TD cell
              if ($cell['is_double_first'] && $cell['double_group_id']) {
                  $partner = $dsPartnerByEntry[(int)$cell['entry_id']] ?? null;
                  if ($partner) {
                      $dsRenderedAsPartner[(int)$partner['entry_id']] = true;
                      $pss = $partner['sub_status'] ?? null;
                      $pCls = match(true) {
                          $pss === 'conflict'        => 'sub-conflict',
                          $pss === 'pending_confirm' => 'sub-pending',
                          $pss === 'claimed'         => 'sub-claimed',
                          $pss === 'confirmed'       => 'sub-confirmed',
                          $pss === 'open'            => 'sub-open',
                          default                    => 'sub-normal',
                      };
                      $pIsMine = $myTeacherId && (int)$partner['teacher_id'] === (int)$myTeacherId;
                      $pIsConf = $pss === 'confirmed';
                      $pCanRelease = !$isFree
                          && (!$pss || ($pIsConf && Auth::isVerwaltung()))
                          && (Auth::isVerwaltung() || ($pIsMine && Auth::isLehrer() && !$pss));
                      $pCanClaim = $pss === 'open' && !$pIsMine
                          && (Auth::isLehrer() || Auth::isVerwaltung());
                      $pCanSelfAssign = !$isFree && !$pIsMine
                          && (!$pss || ($pIsConf && Auth::isVerwaltung()))
                          && (Auth::isLehrer() || Auth::isVerwaltung());
                      $pCanConfirmMe = $pss === 'pending_confirm' && $pIsMine;
                      $pCanAdminOK   = Auth::isVerwaltung() && $pss === 'pending_confirm';
                      $pCanReassign  = Auth::isVerwaltung() && $pIsConf;
                      $pSlotStart    = $partner['time_start'];
              ?>

              <hr style="margin:4px 0;border-color:rgba(0,0,0,.12)">
              <div class="lesson-slot <?=$pCls?> <?=$pIsMine?'mine':''?>">
                <div class="fw-semibold text-truncate" style="font-size:.72rem">
                  <?= e($partner['subject_name']) ?>
                  <span class="badge bg-secondary" style="font-size:.5rem">DS2</span>
                  <span class="text-muted fw-normal" style="font-size:.6rem"><?= substr($pSlotStart,0,5) ?></span>
                </div>
                <?php if ($mode === 'klasse'): ?>
                <div class="text-muted text-truncate" style="font-size:.65rem"><?= e($partner['teacher_name']) ?></div>
                <?php else: ?>
                <div class="text-muted text-truncate" style="font-size:.65rem"><?= e($partner['classes'] ?? '') ?></div>
                <?php endif; ?>
                <?php if ($pss === 'open'): ?>
                  <span class="badge bg-warning text-dark mt-1" style="font-size:.58rem">Offen</span>
                <?php elseif ($pss === 'pending_confirm'): ?>
                  <div class="mt-1" style="font-size:.62rem;color:#92400e">⏳ <?= e($partner['substitute_name'] ?? '?') ?> (warte auf OK)</div>
                <?php elseif ($pss === 'claimed' || $pss === 'confirmed'): ?>
                  <div class="mt-1" style="font-size:.62rem;color:#15803d">✓ <?= e($partner['substitute_name'] ?? '') ?>
                    <?php if ($pss==='confirmed'): ?><span class="badge bg-success ms-1" style="font-size:.52rem">Bestätigt</span><?php endif; ?>
                  </div>
                <?php elseif ($pss === 'conflict'): ?>
                  <div class="mt-1" style="font-size:.62rem;color:#dc2626">⚠ Konflikt: <?= e($partner['substitute_name'] ?? '') ?></div>
                <?php endif; ?>
                <div class="mt-1 d-flex flex-wrap gap-1">
                  <?php if ($pCanRelease): ?>
                  <button type="button" class="btn btn-xs btn-outline-warning"
                          onclick="openRelease(<?=(int)$partner['entry_id']?>,'<?=$ds?>',
                            '<?=addslashes($partner['subject_name'])?>',
                            '<?=addslashes($partner['teacher_name'])?>',
                            '<?=$ds.' '.substr($pSlotStart,0,5)?>',
                            0,1,<?=(int)($partner['double_partner_entry_id']??0)?>,
                            '<?=substr($partner['double_partner_time_start']??'',0,5)?>')">
                    <i class="bi bi-unlock-fill"></i> Freigeben
                  </button>
                  <?php endif; ?>
                  <?php if ($pCanClaim): ?>
                  <button type="button" class="btn btn-xs btn-outline-success"
                          onclick="openClaim(<?=(int)$partner['sub_id']?>,'<?=addslashes($partner['subject_name'])?>','<?=$ds.' '.substr($pSlotStart,0,5)?>')">
                    <i class="bi bi-hand-index-fill"></i> Übernehmen
                  </button>
                  <?php endif; ?>
                  <?php if ($pCanSelfAssign): ?>
                  <button type="button" class="btn btn-xs btn-outline-primary"
                          onclick="openSelfAssign(<?=(int)$partner['entry_id']?>,'<?=$ds?>',
                            '<?=addslashes($partner['subject_name'])?>',
                            '<?=addslashes($partner['teacher_name'])?>')">
                    <i class="bi bi-person-plus-fill"></i> Vertreten
                  </button>
                  <?php endif; ?>
                  <?php if ($pCanConfirmMe): ?>
                  <button type="button" class="btn btn-xs btn-success" onclick="doAction('confirm',<?=(int)$partner['sub_id']?>)"><i class="bi bi-check-lg"></i> OK</button>
                  <button type="button" class="btn btn-xs btn-outline-danger" onclick="doAction('reject',<?=(int)$partner['sub_id']?>)"><i class="bi bi-x-lg"></i> Ablehnen</button>
                  <?php endif; ?>
                  <?php if ($pCanAdminOK): ?>
                  <button type="button" class="btn btn-xs btn-outline-success"
                          onclick="openAdminAssign(<?=(int)$partner['sub_id']?>,'<?=addslashes($partner['subject_name'])?>','<?=$ds?>')">
                    <i class="bi bi-shield-check"></i> Bestätigen
                  </button>
                  <?php endif; ?>
                  <?php if ($pCanReassign): ?>
...
<div class="modal fade" id="releaseModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="POST" action="<?= APP_URL ?>/substitutions/action.php" class="modal-content">
      ...
      <input type="hidden" name="double_scope"     id="rDoubleScope" value="single">
      <input type="hidden" name="partner_entry_id" id="rPartnerEntryId" value="">
      ...
      <div class="modal-body">
        ...
        <!-- Doppelstunden-Auswahl -->
        <div id="rDoubleChoice" class="mb-3" style="display:none">
          <label class="form-label fw-semibold">
            <i class="bi bi-layers me-1"></i>Doppelstunde – welche Stunden freigeben?
          </label>
          <div class="d-flex flex-column gap-2">
            <label class="d-flex align-items-center gap-2 p-2 rounded border" style="cursor:pointer">
              <input type="radio" name="ds_choice" value="both" id="rChoiceBoth" onchange="setDoubleScope('both')">
              <span><strong>Beide Stunden</strong> <span class="text-muted small" id="rBothLabel"></span></span>
            </label>
            <label class="d-flex align-items-center gap-2 p-2 rounded border" style="cursor:pointer">
              <input type="radio" name="ds_choice" value="first" id="rChoiceFirst" onchange="setDoubleScope('first')">
              <span><strong>Nur 1. Stunde</strong> <span class="text-muted small" id="rFirstLabel"></span></span>
            </label>
            <label class="d-flex align-items-center gap-2 p-2 rounded border" style="cursor:pointer">
              <input type="radio" name="ds_choice" value="second" id="rChoiceSecond" onchange="setDoubleScope('second')">
              <span><strong>Nur 2. Stunde</strong> <span class="text-muted small" id="rSecondLabel"></span></span>
            </label>
          </div>
          <div class="alert alert-info mt-2 mb-0 py-2 small">
            <i class="bi bi-info-circle me-1"></i>
            Die nicht freigegebene Stunde bleibt offen – kein Vertreter nötig.
          </div>
        </div>
...
<script>
function openRelease(eid, date, subject, teacher, datestr, isFirst, isSecond, partnerEid, partnerTime) {
    ...
    const isDouble = isFirst || isSecond;
    const dsDiv    = document.getElementById('rDoubleChoice');
    dsDiv.style.display = isDouble ? '' : 'none';

    if (isDouble) {
        // Bestimme Zeiten von DS1 und DS2
        // isFirst=1 → diese Stunde IST DS1, Partner ist DS2
        // isSecond=1 → diese Stunde IST DS2, Partner ist DS1
        const thisTime    = datestr.split(' ')[1] || '';
        const p1Time = isFirst  ? thisTime   : partnerTime;
        const p2Time = isSecond ? thisTime   : partnerTime;
        const p1Eid  = isFirst  ? eid        : partnerEid;
        const p2Eid  = isSecond ? eid        : partnerEid;

        document.getElementById('rFirstLabel').textContent  = p1Time ? '(' + p1Time + ' Uhr)' : '';
        document.getElementById('rSecondLabel').textContent = p2Time ? '(' + p2Time + ' Uhr)' : '';
        document.getElementById('rBothLabel').textContent   = p1Time && p2Time ? '(' + p1Time + ' + ' + p2Time + ' Uhr)' : '';

        // Reset to "both"
        document.getElementById('rChoiceBoth').checked = true;
        setDoubleScope('both', p1Eid, p2Eid, partnerEid);

        // Store context for setDoubleScope
        document.getElementById('releaseModal').dataset.p1Eid = p1Eid;
        document.getElementById('releaseModal').dataset.p2Eid = p2Eid;
        document.getElementById('releaseModal').dataset.partnerEid = partnerEid;
        document.getElementById('releaseModal').dataset.isFirst = isFirst;
    } else {
        document.getElementById('rDoubleScope').value    = 'single';
        document.getElementById('rPartnerEntryId').value = '';
    }

    new bootstrap.Modal(document.getElementById('releaseModal')).show();
}

function setDoubleScope(scope) {
    const modal   = document.getElementById('releaseModal');
    const isFirst = parseInt(modal.dataset.isFirst || '0');
    const partnerEid = modal.dataset.partnerEid;

    document.getElementById('rDoubleScope').value = scope;

    if (scope === 'both') {
        // entry_id bleibt gesetzt (DS1 oder DS2 je nach Klick), partner wird mitgeschickt
        document.getElementById('rPartnerEntryId').value = partnerEid;
    } else if (scope === 'first') {
        // Nur DS1 freigeben: entry_id = DS1-Entry
        const p1Eid = modal.dataset.p1Eid;
        document.getElementById('rEntryId').value       = p1Eid;
        document.getElementById('rPartnerEntryId').value = '';
    } else if (scope === 'second') {
        // Nur DS2 freigeben: entry_id = DS2-Entry
        const p2Eid = modal.dataset.p2Eid;
        document.getElementById('rEntryId').value       = p2Eid;
        document.getElementById('rPartnerEntryId').value = '';
    }
}
